...
describe('WsMetricRegistry', function () {
  ...
  describe('metric properties', function () {
    it('should have correct metric names from the metric catalog', function () {
      ...
      expect((methodsCounter as any).name).to.equal(METRICS.ws.methodsCounter.name);
      expect((connectionDuration as any).name).to.equal(METRICS.ws.connectionDuration.name);
    });
    ...
    it('should have correct buckets for histograms', function () {
      ...
      expect((connectionDuration as any).buckets).to.deep.equal(METRICS.ws.connectionDuration.buckets);
      expect((messageDuration as any).buckets).to.deep.equal(METRICS.ws.messageDuration.buckets);
    });
    ...
    it('should have correct label names for metrics with labels', function () {
      ...
      expect((methodsCounter as any).labelNames).to.deep.equal(METRICS.ws.methodsCounter.labelNames);
      expect((methodsCounterByIp as any).labelNames).to.deep.equal(METRICS.ws.methodsCounterByIp.labelNames);
    });
  });
