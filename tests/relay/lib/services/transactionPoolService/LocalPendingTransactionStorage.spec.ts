...
describe('LocalPendingTransactionStorage Test Suite', function () {
  ...
  describe('getList', () => {
    ...
    it('should return correct count for address with pending transactions', async () => {
      ...
      await storage.addToList(testAddress1, testRlp1, 0);
    ...
    it('should return correct count after multiple transactions added', async () => {
      ...
      await storage.addToList(testAddress1, testRlp1, 0);
      await storage.addToList(testAddress1, testRlp2, 0);
    ...
    it('should return different counts for different addresses', async () => {
      ...
      await storage.addToList(testAddress1, testRlp1, 0);
      await storage.addToList(testAddress2, testRlp2, 0);
      await storage.addToList(testAddress2, testRlp3, 0);
  ...
  describe('removeFromList', () => {
    ...
    it('should remove specific transaction from multiple transactions', async () => {
      ...
      await storage.addToList(testAddress1, testRlp1, 0);
      await storage.addToList(testAddress1, testRlp2, 0);
      await storage.addToList(testAddress1, testRlp3, 0);
  ...
  describe('getConfirmedCount should return null for address with no cached baseline', () => {
    [
      ...
      },
      {
        ...
        run: async () => {
          await storage.addToList(testAddress1, testRlp1, 5);
        },
        address: testAddress1,
      },
      {
        ...
        run: async () => {
          await storage.addToList(testAddress1, testRlp1, 3);
          await storage.removeFromList(testAddress1, testRlp1);
        },
        address: testAddress1,
      },
      {
        ...
        run: async () => {
          await storage.addToList(testAddress2, testRlp2, 10);
          await storage.addToList(testAddress2, testRlp3, 11);
  ...
  describe('Integration scenarios', () => {
    ...
    it('should handle multiple addresses with different transaction counts', async () => {
      ...
      await storage.addToList(addresses[0], testRlp1, 0);
      await storage.addToList(addresses[0], testRlp2, 0);
      await storage.addToList(addresses[0], testRlp3, 0);

      await storage.addToList(addresses[1], testRlp1, 0);

      await storage.addToList(addresses[2], testRlp1, 0);
      await storage.addToList(addresses[2], testRlp2, 0);
  ...
  describe('Edge cases', () => {
    it('should handle large transaction counts', async () => {
      ...
      for (let i = 0; i < largeCount; i++) {
        ...
        await storage.addToList(testAddress1, rlp, 0);
      }
