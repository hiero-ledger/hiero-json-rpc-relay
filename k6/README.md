...
| DEFAULT_DURATION      | Duration of each test, K6 will perform as many requests as possible for each test in this given timeframe. Some tests might override this duration to a minimum for at least 1 successful call for that test. i.e: if DEFAULT_DURATION=1s, but `eth_sendRawTransaction` needs at least 3s, k6 will use the bigger of the 2 (3s). to try to guarantee at least 1 successful call. When staged execution is used (RAMP_UP_DURATION, STABLE_DURATION, RAMP_DOWN_DURATION are set), this value is ignored in favor of the staged configuration. | 120s          | false    |
| RAMP_UP_DURATION      | Duration for the ramp-up phase in staged execution. Gradually increases load from 0 to target VUs. If set along with other staging variables, enables staged execution instead of using DEFAULT*DURATION. \_Only applicable for [Stress Testing](#stress-test)*                                                                                                                                                                                                                                                                             |               | false    |
| STABLE_DURATION       | Duration for the stable phase in staged execution. Maintains steady load at target VUs. If set along with other staging variables, enables staged execution instead of using DEFAULT*DURATION. \_Only applicable for [Stress Testing](#stress-test)*                                                                                                                                                                                                                                                                                        |               | false    |
| RAMP_DOWN_DURATION    | Duration for the ramp-down phase in staged execution. Gradually decreases load back to 0 VUs. If set along with other staging variables, enables staged execution instead of using DEFAULT*DURATION. \_Only applicable for [Stress Testing](#stress-test)*                                                                                                                                                                                                                                                                                  |               | false    |
...
| TEST_TYPE             | Type of test to run: `performance` (sequential), `load` (concurrent), or `stress` (staged execution with realistic VU distribution). When set to `stress`, automatically enables staged execution with default durations if staging variables are not explicitly set.                                                                                                                                                                                                                                                                       | performance   | false    |
...
The stress test simulates realistic traffic patterns using 90 days of production data with staged execution for realistic load patterns. Virtual users (VUs) are distributed across endpoints to reflect actual usage proportions.

### Test Execution Model

**Staged Execution**: The stress test uses a ramping-vus executor with three phases:

- **Ramp-up phase**: Gradually increases load from 0 to target VUs over the specified duration
- **Stable phase**: Maintains steady load at target VUs for sustained testing
- **Ramp-down phase**: Gradually decreases load back to 0 VUs for graceful shutdown

This staged approach provides more realistic load testing that better matches real-world traffic patterns, avoiding sudden load spikes and drops.
...
MIRROR_BASE_URL=
RELAY_BASE_URL=
# Option 1: Use TEST_TYPE=stress for automatic staging with defaults
TEST_TYPE=stress         # Automatically enables staged execution with default timings
# Option 2: Explicit staging configuration (optional - uses TEST_TYPE=stress defaults if not specified)
RAMP_UP_DURATION=30s     # Duration to ramp up to target VUs
STABLE_DURATION=60s      # Duration to maintain target VUs
RAMP_DOWN_DURATION=30s   # Duration to ramp down to 0 VUs
# Common configuration
DEFAULT_VUS=30           # Target number of virtual users
SIGNED_TXS=30            # Increase SIGNED_TXS for longer test duration
WALLET_BALANCE=30        # Increase WALLET_BALANCE for longer test duration
```

#### Preparing Artifacts

As with other tests, the stress test requires preparation data (smart contracts, wallets, signed transactions, filter IDs). It's advisable to run the prep script before every time running stress test:
...
- A `stress-test-report.md` file with detailed metrics including staging information when applicable
