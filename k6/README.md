...
FILTER_TEST=*            # Endpoints to include (comma-separated or * for all)
```
...
```shell
npm run stress-test --env FILTER_TEST=eth_chainId,eth_getBlockByNumber,eth_getLogs
```

Run with custom VU allocation:

```shell
npm run stress-test --env DEFAULT_VUS=200 --env DEFAULT_DURATION=120s
```
