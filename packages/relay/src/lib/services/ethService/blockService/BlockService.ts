...
export class BlockService implements IBlockService {
  ...
  /**
   * Worker pool for offloading CPU-intensive block processing operations.
   * @private
   */
  ...
  constructor(
    ...
    this.workerPool = new Piscina({
      filename: resolve(__dirname, 'blockWorker.js'),
      minThreads: 2,
      maxThreads: 4,
    });

    logger.info('Initialized BlockService with worker pool for CPU-intensive operations');
  }
