...
import { IMethodRateLimitConfiguration } from '@hashgraph/json-rpc-relay/dist/lib/types';
...
export default class KoaJsonRpc {
  ...
  constructor(logger: Logger, register: Registry, relay: Relay, opts?: { limit: string | null }) {
    ...
    this.logger = logger;
    this.rateLimiter = new RateLimiterService(logger.child({ name: 'ip-rate-limit' }), register, this.duration);
  ...
  async getRequestResult(request: IJsonRpcRequest, ip: string): Promise<IJsonRpcResponse> {
    try {
      ...
      if (await this.rateLimiter.shouldRateLimit(ip, request.method, methodTotalLimit, this.requestId)) {
