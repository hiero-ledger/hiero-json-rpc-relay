...
# Post-build trace using @vercel/nft to generate a purely standalone build with only executed files.
# This eliminates the need manually delete dev/optional dependencies, documentation, tests,
# and heavy modules like react-native that are not used in a backend server context.
...
# Copy only the dynamically traced standalone files from the build stage.
