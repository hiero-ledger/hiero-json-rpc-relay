...
describe('StargateHTSConnectorExistingToken', function() {
  it('@hedera @update-keys', async () => {
    const contract = await ethers.getContractAt('CreateHTS', process.env.STARGATE_HTS_CONNECTOR_CREATE_HTS_CONTRACT);
    const updatedKey = [
      false,
  ...
  it('@hedera @fund', async () => {
    const oftHts = await ethers.getContractAt('ExampleStargateHTSConnectorExistingToken', process.env.STARGATE_HTS_CONNECTOR_EXISTING_TOKEN_HEDERA_CONTRACT);
    const tokenAddress = await oftHts.htsTokenAddress();

    const contract = await ethers.getContractAt('ERC20', tokenAddress);
    const txTransfer = await contract.transfer(process.env.STARGATE_HTS_CONNECTOR_EXISTING_TOKEN_HEDERA_CONTRACT, '200');
  ...
  it('@hedera @approve oft hts contract', async () => {
    ...
    const contract = await ethers.getContractAt('ERC20', tokenAddress);
    const txApprove = await contract.approve(process.env.STARGATE_HTS_CONNECTOR_EXISTING_TOKEN_HEDERA_CONTRACT, amount);
  ...
  it('@hedera @test balance', async () => {
    ...
    console.log(`(${hre.network.name}) oft contract balance: ${await contract.balanceOf(process.env.STARGATE_HTS_CONNECTOR_EXISTING_TOKEN_HEDERA_CONTRACT)}`);
  ...
  it('@bsc @test balance', async () => {
    ...
    const contract = await ethers.getContractAt('ERC20', process.env.STARGATE_HTS_CONNECTOR_EXISTING_TOKEN_BSC_CONTRACT);
    const receiverBalance = await contract.balanceOf(RECEIVER_ADDRESS);

    console.log(`(${hre.network.name}) oft contract balance: ${await contract.balanceOf(process.env.STARGATE_HTS_CONNECTOR_EXISTING_TOKEN_BSC_CONTRACT)}`);
    console.log(`(${hre.network.name}) signer balance: ${await contract.balanceOf(signers[0].address)}`);
    console.log(`(${hre.network.name}) total supply: ${await contract.totalSupply()}`);
    console.log(`(${hre.network.name}) receiver balance: ${receiverBalance}`);
