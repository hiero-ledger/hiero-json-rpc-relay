...
export class TransactionService implements ITransactionService {
  ...
  async getTransactionReceipt(hash: string, requestDetails: RequestDetails): Promise<ITransactionReceipt | null> {
    ...
    const receiptResponse = await this.mirrorNodeClient.getContractResultWithRetry<any | null>(
      this.mirrorNodeClient.getContractResult.name,
      [hash, requestDetails],
      { returnImmatureRecords: true },
    );
   ...
   */
  private async buildRejectedRecordError(hash: string, record: any): Promise<JsonRpcError> {
