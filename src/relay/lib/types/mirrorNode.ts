...
import type { AccessListEntry, AuthorizationListEntry } from '../model';
...
/**
 * Common contract-result fields shared by every mirror node contract-result response, and the exact
 * shape returned by the `/contracts/{contractIdOrAddress}/results` (per-contract list) endpoint.
 */
export interface MirrorNodeContractResultBase {
...
/**
 * Represents a contract result as returned by the Hedera Mirror Node `/contracts/results` endpoint.
 */
export interface MirrorNodeContractResult extends MirrorNodeContractResultBase {
...
/**
 * Represents a single contract state change as returned by the Hedera Mirror Node on a detailed
 * contract result (`/contracts/results/{transactionIdOrHash}`).
 */
export interface MirrorNodeContractResultStateChange {
  /** The address of the contract whose state changed. */
  address: string;
  /** The unique identifier of the contract. */
  contract_id: string;
...
/**
 * Represents a log nested in a detailed contract result returned by the Hedera Mirror Node
 * `/contracts/results/{transactionIdOrHash}` endpoint.
 */
export interface MirrorNodeContractResultLog {
  /** The address of the contract that emitted the log. */
  address: string;
  /** The bloom filter for the log. */
  bloom: string;
...
/**
 * Represents a contract result as returned by the Hedera Mirror Node `/contracts/results/{transactionIdOrHash}` endpoint.
 */
export interface MirrorNodeContractResultDetails extends MirrorNodeContractResult {
  /** The logs emitted during the contract execution. */
  logs: MirrorNodeContractResultLog[];
  /** The contract state changes produced by the execution. */
  state_changes: MirrorNodeContractResultStateChange[];
}

/**
 * A contract result as consumed by the transaction-receipt builders: a {@link MirrorNodeContractResult}
 * plus an optional legacy `root` field.
 */
export interface MirrorNodeContractResultReceipt extends MirrorNodeContractResult {
  /**
   * Legacy (pre-Byzantium) receipt state root. The Hedera Mirror Node does not return this field; the
   * receipt builders read it defensively and fall back to a default, so it is optional.
   */
  root?: string;
}

/**
 * A paginated page of contract results as returned by the Hedera Mirror Node
 * `/contracts/{contractIdOrAddress}/results` endpoint.
 */
export interface MirrorNodeContractResultsPage {
  /** The contract results on this page. */
  results: MirrorNodeContractResultBase[];
  /** Pagination links; `next` is the path to the next page, or null when there are no more pages. */
